...
const ProductAll = () => {
  ...
  // 비동기처리
  // 특정 코드의 실행이 완료될 때 까지 기다리지 않고 다음 코드를 먼저 실행한다.

  // 1. 함수 앞에 async 예약어를 써준다.
  // 2. 함수 내부 로직 중 HTTP 통신을 하는 비동기 처리 코드 앞에 await 작성
  // 3. await 함수 내에서 사용하려면, 반드시 함수 앞에 async 키워드가 있어야 한다.

  // await
  // 함수를 호출하고 본문이 실행될 때 잠시 중단 됐다가 프라미스가 처리되면 실행이 재개된다.
  // 프라미스 : 자바스크립트 비동기 처리에 사용되는 객체
   
  const getProducts = async () => {
    // 모든 데이터를 가진 주소를 부른다.
    let url = " http://localhost:5000/produts"
    // url을 fetch 해줘라
